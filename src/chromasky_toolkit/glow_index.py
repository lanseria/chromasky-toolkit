...
class GlowIndexCalculator:
    ...
    # --- 核心计算逻辑 ---
    def calculate_for_point(self, lat: float, lon: float, utc_time: datetime) -> Dict[str, float]:
        """
        ...
        # 4. 组合所有因子得出最终分数
        #    所有因子相乘，确保任何一项为0都会拉低总分
        final_score = score_boundary * score_hcc * score_mcc * score_lcc
    ...
    # 删除了一个重复的 _find_cloud_boundary_distance 方法
    def _find_cloud_boundary_distance(self, start_lat: float, start_lon: float, sun_azimuth_deg: float) -> float:
        ...
        if boundary_indices.size > 0:
            return distances[boundary_indices[0]]
        else:
    ...
    def calculate_for_grid(self, utc_time: datetime, active_mask: xr.DataArray) -> xr.Dataset:
        """为网格中的活动区域计算火烧云指数，并返回包含所有分数的 Dataset。"""
        logging.info("开始为网格活动区域计算多因子火烧云指数...")
        ...
        with tqdm(total=len(active_indices), desc="Calculating Multi-Factor Glow Index") as pbar:
            for i, j in active_indices:
                lat, lon = lats.values[i], lons.values[j]
                scores = self.calculate_for_point(lat, lon, utc_time)
